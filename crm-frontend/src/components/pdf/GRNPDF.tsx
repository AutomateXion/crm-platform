...
export default function GRNPDF({ data, companyInfo, config }: Props) {
  ...
  useEffect(() => {
    let active = true;
    generateQRCode({
      'Doc Type': 'GOODS RECEIPT NOTE', 'Number': data.grnNumber,
      'Date': data.grnDate ? new Date(data.grnDate).toLocaleDateString() : '',
      'Supplier': data.supplierName, 'Status': data.status,
  ...
  const pageIds = chunks.map((_, i) => `grn-page-${i}`);
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, borderBottom: `3px solid ${C}`, paddingBottom: 10 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: C, marginBottom: 4 }}>GOODS RECEIPT NOTE</div>
          <div style={{ fontSize: 10, color: '#666' }}>
            <strong>{data.grnNumber}</strong><br />
            Date: {data.grnDate ? new Date(data.grnDate).toLocaleDateString() : '—'}<br />
            {show('poRef') && data.poNumber && <>PO Ref: {data.poNumber}<br /></>}
      ...
      <div style={{ marginBottom: 8 }}>
        ...
        {show('deliveryNoteRef') && data.deliveryNoteRef && <span style={{ color: '#666', fontSize: 9 }}>  ·  DN Ref: {data.deliveryNoteRef}</span>}
      ...
      {cfg.showSignature && (
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16 }}>
          <div style={{ textAlign: 'center', width: 200 }}>
            ...
            <div style={{ fontSize: 9, color: '#666', marginTop: 3 }}>{show('receivedBy') && data.receivedBy ? data.receivedBy : '________________'}</div>
          </div>
          <div style={{ textAlign: 'center', width: 200 }}>
            <div style={{ borderTop: '1px solid #333', paddingTop: 5, fontSize: 9 }}>Checked / Approved By</div>
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16 }}>
        ...
        <Button type="primary" icon={<DownloadOutlined />} onClick={() => downloadMultiPagePDF(pageIds, `${data.grnNumber}.pdf`)}>Download PDF</Button>
      </Space>
      ...
      {chunks.map((chunk, p) => {
        ...
        return (
          <div id={`grn-page-${p}`} key={p} style={pageBoxStyle}>
            <Header pageNo={p + 1} />
            <div style={{ flex: 1 }}>
            ...
            <div style={{ borderTop: `2px solid ${C}`, paddingTop: 8, marginTop: 10, fontSize: 8, color: '#999', display: 'flex', justifyContent: 'space-between' }}>
              ...
              <div>{cfg.footerNote || 'Computer-generated Goods Receipt Note'}</div>
