...
interface InvoicePDFProps { data: any; companyInfo?: any; }
      ...
      <div id="invoice-pdf-content" style={{ width: '210mm', minHeight: '297mm', background: '#fff', padding: '15mm', fontFamily: 'Arial, sans-serif', fontSize: '10pt', color: '#333', margin: '0 auto', border: '1px solid #e0e0e0' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, borderBottom: '3px solid #52c41a', paddingBottom: 16 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: 11, color: '#666' }}>
              ...
              Due: {data.dueDate ? new Date(data.dueDate).toLocaleDateString() : '—'}<br />
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
          <div style={{ flex: 1 }}>
            ...
            {data.customerTrn && <div style={{ color: '#666', fontSize: 10, fontWeight: 600 }}>TRN: {data.customerTrn}</div>}
          </div>
          <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 24 }}>
          <table style={{ width: 300 }}>
            <tbody>
              ...
              <tr style={{ borderBottom: '1px solid #eee' }}><td style={{ padding: '5px 10px', fontSize: 10, color: '#666' }}>VAT ({data.vatRate || 5}%):</td><td style={{ padding: '5px 10px', textAlign: 'right', fontSize: 10 }}>OMR {Number(data.vatAmount || 0).toFixed(3)}</td></tr>
              <tr style={{ background: '#52c41a', color: '#fff' }}>
        ...
        {/* Payment Terms */}
        {data.paymentTerms && <div style={{ fontSize: 10, color: '#666', marginBottom: 8 }}><strong>Payment Terms:</strong> {data.paymentTerms}</div>}
        {data.notes && <div style={{ fontSize: 10, color: '#666', marginBottom: 16, borderTop: '1px solid #eee', paddingTop: 8 }}><strong>Notes:</strong> {data.notes}</div>}
