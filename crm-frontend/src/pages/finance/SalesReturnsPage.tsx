...
import { Table, Card, Button, Tag, Space, Modal, Form, Input, Typography, Row, Col, message, Popconfirm, Select, InputNumber, Switch, Divider, AutoComplete } from 'antd';
import { PlusOutlined, DeleteOutlined, EditOutlined } from '@ant-design/icons';
import { salesReturnsApi, invoicesApi } from '../../services/salesApi';
...
export default function SalesReturnsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const r = await salesReturnsApi.getAll({ page, limit: 20 }); setItems(r.data.data || []); setTotal(r.data.total || 0); }
    catch {} finally { setLoading(false); }
  }, [page]);

  useEffect(() => { load(); }, [load]);

  const defaultLine = () => ({ description: '', quantity: 1, unitPrice: 0, lineTotal: 0, isTaxable: true, unitOfMeasure: 'PCS' });

  const openCreate = () => {
    setEditRecord(null); form.resetFields();
    form.setFieldsValue({ status: 'DRAFT', isInventory: true, returnDate: new Date().toISOString().slice(0,10) });
    setLineItems([defaultLine()]);
    setFilteredInvoices([]);
    // Load all invoices fresh
    invoicesApi.getAll({ limit: 500 }).then(r => setAllInvoices(r.data.data || [])).catch(() => {});
    setModalOpen(true);
  };
  ...
  const handleAccountSearch = async (val: string) => {
    ...
    try {
      ...
      setAccountOptions((Array.isArray(r.data) ? r.data : (r.data.data || [])).map((a: any) => ({ value: a.accountName, label: a.accountName, account: a })));
    } catch {}
  };
  ...
  const handleCustomerSelect = async (_: string, option: any) => {
    if (option.account) {
      ...
      form.setFieldsValue({ customerName, invoiceId: undefined });
      setLineItems([defaultLine()]);
      // Fetch invoices for this customer fresh from backend
      try {
        const r = await invoicesApi.getAll({ limit: 500 });
        const all = r.data.data || [];
        setAllInvoices(all);
        const customerInvoices = all.filter((i: any) => i.customerName === customerName);
  ...
  const handleInvoiceSelect = async (invoiceId: string) => {
    ...
    try {
      const r = await invoicesApi.getOne(invoiceId);
      const inv = r.data;
      form.setFieldsValue({ customerName: inv.customerName });
      // Warn if invoice is already fully paid
      if (inv.status === 'PAID' || Number(inv.balanceDue) <= 0) {
        message.warning({
          content: `⚠️ This invoice is already fully paid (OMR ${Number(inv.totalAmount).toFixed(3)}). The return will create a Credit Balance for ${inv.customerName}. You can apply this credit to another invoice or issue a refund.`,
          duration: 6,
        });
      }
  ...
  const updateLine = (idx: number, field: string, value: any) => {
    ...
    updated[idx].lineTotal = Number(updated[idx].quantity || 1) * Number(updated[idx].unitPrice || 0) * (1 - Number(updated[idx].discountPct || 0) / 100);
  ...
  const handleSave = async (values: any) => {
    ...
    try {
      ...
      if (editRecord) await salesReturnsApi.update(editRecord.returnId, payload);
      else await salesReturnsApi.create(payload);
  ...
  const columns = [
    ...
    { title: 'Total', dataIndex: 'totalAmount', render: (v: number) => <Text strong style={{ color: '#ff4d4f' }}>OMR {Number(v).toFixed(3)}</Text> },
    { title: 'Status', dataIndex: 'status', render: (v: string) => <Tag color={STATUS_COLORS[v]}>{v}</Tag> },
    { title: 'Credit Note', dataIndex: 'creditNoteNumber', render: (v: string) => v ? <Tag color="purple">{v}</Tag> : '—' },
    { title: '', key: 'actions', render: (_: any, r: any) => (
      <Space>
        {r.status === 'DRAFT' && <Button size="small" type="primary" style={{background:'#1890ff'}} onClick={async () => { await sApi.patch(`/sales/returns/${r.returnId}/status`, { status: 'APPROVED' }); load(); message.success('Approved!'); }}>Approve</Button>}
        {r.status === 'APPROVED' && <Button size="small" type="primary" style={{background:'#52c41a'}} onClick={async () => { await sApi.patch(`/sales/returns/${r.returnId}/status`, { status: 'PROCESSED' }); load(); message.success('Processed!'); }}>Process</Button>}
        ...
        <Popconfirm title="Delete return?" onConfirm={async () => { await salesReturnsApi.delete(r.returnId); load(); }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
        <div><Title level={4} style={{ margin: 0 }}>Sales Returns</Title><Text type="secondary">Manage customer returns and credit notes</Text></div>
      ...
      <Modal title={editRecord ? 'Edit Return' : 'New Sales Return'} open={modalOpen} onCancel={() => setModalOpen(false)} footer={null} width={900} style={{ top: 20 }}>
        <Form form={form} layout="vertical" onFinish={handleSave} style={{ marginTop: 12 }}>
          <Row gutter={12}>
            ...
            <Col span={10}>
              <Form.Item name="customerName" label="Step 1: Select Customer" rules={[{ required: true }]}>
                <AutoComplete options={accountOptions} onSearch={handleAccountSearch} onSelect={handleCustomerSelect}
                  placeholder="Search customer name..." allowClear />
              </Form.Item>
            </Col>
            {/* Step 2: Select Invoice */}
            <Col span={10}>
              <Form.Item name="invoiceId" label="Step 2: Select Invoice to Return">
                <Select allowClear showSearch optionFilterProp="children" placeholder="Select customer first..."
                  onChange={handleInvoiceSelect}>
                  {(filteredInvoices.length > 0 ? filteredInvoices : allInvoices).map((i: any) => (
                    <Option key={i.invoiceId} value={i.invoiceId}>
                      {i.invoiceNumber} — {i.customerName} — OMR {Number(i.totalAmount).toFixed(3)}
                    </Option>
                  ))}
                </Select>
              </Form.Item>
          ...
          <Row gutter={12}>
            ...
            <Col span={6}><Form.Item name="isInventory" label="Return to Stock" valuePropName="checked"><Switch checkedChildren="Yes" unCheckedChildren="No" /></Form.Item></Col>
            <Col span={6}><Form.Item name="creditNoteNumber" label="Credit Note #"><Input /></Form.Item></Col>
          ...
          <Row gutter={8} style={{ marginBottom: 8, fontWeight: 600, color: '#8c8c8c', fontSize: 12 }}>
            <Col span={1}>#</Col><Col span={7}>Description</Col><Col span={2}>UOM</Col>
          ...
          {lineItems.map((item, idx) => (
            <Row gutter={8} key={idx} style={{ marginBottom: 8 }} align="middle">
              ...
              <Col span={7}><Input value={item.description} onChange={e => updateLine(idx, 'description', e.target.value)} placeholder="Description" /></Col>
              ...
              <Col span={4}><Text strong style={{ color: '#ff4d4f' }}>OMR {Number(item.lineTotal).toFixed(3)}</Text></Col>
          ...
          <Row justify="end">
            <Col span={8}>
              <div style={{ background: '#fff1f0', padding: 12, borderRadius: 8, border: '1px solid #ffccc7' }}>
                ...
                <Row justify="space-between"><Text strong style={{ fontSize: 15 }}>Total Credit:</Text><Text strong style={{ fontSize: 15, color: '#ff4d4f' }}>OMR {Number(form.getFieldValue('totalAmount')||0).toFixed(3)}</Text></Row>
              </div>
            </Col>
          </Row>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
            <Button onClick={() => setModalOpen(false)}>Cancel</Button>
            <Button type="primary" htmlType="submit" loading={saving} danger>{editRecord ? 'Save' : 'Create Return'}</Button>
          </div>
        </Form>
      </Modal>
    </div>
