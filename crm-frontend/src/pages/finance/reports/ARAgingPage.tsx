...
export default function ARAgingPage() {
  ...
  const load = async () => {
    setLoading(true);
    try { const r = await salesApi.get('/sales/ar-aging', { params: { asOfDate: asOfDate||undefined } }); setData(r.data); setSearched(true); }
  ...
  const columns = [
    ...
    { title:'Balance (OMR)', dataIndex:'balanceDue', align:'right' as const, render:(v:number) => <Text strong style={{color:'#ff4d4f'}}>OMR {Number(v).toFixed(3)}</Text> },
  ];

  const { settings: company } = useCompanySettings();

  const PDFContent = () => (
    <div id="ar-aging-pdf" style={{ width:'210mm', background:'#fff', padding:'12mm 15mm', fontFamily:'Arial, sans-serif', fontSize:'9pt', color:'#333' }}>
      {/* Header */}
      <div style={{ display:'flex', justifyContent:'space-between', borderBottom:'3px solid #1890ff', paddingBottom:14, marginBottom:20 }}>
        ...
        <div style={{ textAlign:'right' }}>
          <div style={{ fontSize:16, fontWeight:700, color:'#1890ff' }}>AR AGING REPORT</div>
      ...
      <table style={{ width:'100%', borderCollapse:'collapse', fontSize:9 }}>
        <thead>
          <tr style={{ background:'#1890ff', color:'#fff' }}>
            {['Customer','Invoice #','Invoice Date','Due Date','Age','Total (OMR)','Paid (OMR)','Balance (OMR)'].map(h => (
              <th key={h} style={{ padding:'7px 8px', textAlign:'left', fontSize:9 }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {(data?.invoices||[]).map((row:any, i:number) => (
            <tr key={i} style={{ background:i%2===0?'#fff':'#f9f9f9', borderBottom:'1px solid #eee' }}>
              ...
              <td style={{ padding:'5px 8px', color:row.daysOverdue<=0?'#52c41a':row.daysOverdue<=30?'#1890ff':'#ff4d4f', fontWeight:600 }}>{row.daysOverdue<=0?'Current':`${row.daysOverdue}d`}</td>
              ...
              <td style={{ padding:'5px 8px', textAlign:'right', color:'#ff4d4f', fontWeight:700 }}>OMR {Number(row.balanceDue||0).toFixed(3)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr style={{ background:'#1890ff', color:'#fff', fontWeight:700 }}>
            <td colSpan={7} style={{ padding:'8px', textAlign:'right', fontSize:11 }}>TOTAL OUTSTANDING</td>
  ...
  return (
    <div id="ar-aging-report">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
        <div><Title level={4} style={{ margin:0 }}>Accounts Receivable Aging</Title><Text type="secondary">Outstanding customer invoices by age</Text></div>
      </div>
      <Card style={{ borderRadius:12, marginBottom:16 }} size="small">
        <Row gutter={12} align="middle">
          ...
          <Col span={3} style={{ paddingTop:22 }}><ExportButton config={{ filename:'ar-aging', data: data?.invoices||[] }} /></Col>
        </Row>
      </Card>
      {searched && data && (
        <>
          ...
          <Card style={{ borderRadius:12 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:12 }}>
              <Text>Total Outstanding: <Text strong style={{ color:'#ff4d4f', fontSize:16 }}>OMR {Number(data.totalOutstanding||0).toFixed(3)}</Text></Text>
      ...
      <Modal open={showPDF} onCancel={() => setShowPDF(false)} footer={null} width="100vw" style={{ top:0, padding:0, maxWidth:'100vw' }} styles={{ body:{ height:'95vh', overflow:'auto', padding:16, background:'#f5f5f5' } }} title="AR Aging Report — PDF Preview">
        <Space style={{ marginBottom:16 }}>
          <Button icon={<PrinterOutlined />} onClick={() => printDocument('ar-aging-pdf')}>Print</Button>
          <Button type="primary" icon={<DownloadOutlined />} onClick={() => downloadPDF('ar-aging-pdf', `ar-aging-${asOfDate}.pdf`)}>Download PDF</Button>
        </Space>
        <PDFContent />
      </Modal>
    </div>
